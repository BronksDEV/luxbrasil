...
import {
  Container, Typography, Paper, Table, TableBody, TableCell, TableHead, TableRow, Tabs, Tab, Box, Button, TextField,
  Switch, FormControlLabel, Chip, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, InputAdornment,
  Select, FormControl, InputLabel, MenuItem, CircularProgress, Alert, ToggleButton, ToggleButtonGroup, Card, CardContent,
  Grid, useMediaQuery, LinearProgress, Stack, Divider, Tooltip, Avatar
} from '@mui/material';
import { useTheme } from '@mui/material/styles';
import {
  Search, Add, Delete, Block, Casino, Refresh, Diamond, Save, CheckCircle, MonetizationOn, Event, Close, Download, Edit,
  LocalOffer, Print, Person, History, Gavel, MoreVert, ExpandMore, FilterList, Group, Bolt, WhatsApp, Construction
...
import AdminIPControl from '../components/AdminIPControl';
...
const Admin: React.FC = () => {
  ...
  const [localThemeConfig, setLocalThemeConfig] = useState<SystemThemeConfig>({
    active: false,
    name: 'default',
  ...
  // Estado para editar/criar prêmio. Se id existir, é edição.
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // A aba 6 (Controle de IP) tem seu próprio carregamento de dados interno
    } catch (e) {
  ...
  const handleSaveTheme = async () => {
    ...
    try {
      ...
      await refreshTheme();
      alert('Configuração do sistema salva e aplicada com sucesso!');
    } catch (e: any) {
...
Somos a equipe da Agência Lux Brasil.
Entramos em contato porque você foi contemplada(o) no sorteio da nossa roleta 🎉

Para darmos andamento na entrega do prêmio, precisamos confirmar alguns dados de envio.
📦 O prazo para envio é de até 90 dias após a confirmação dos dados.
...
Ficamos no aguardo`;
  ...
  const formatLogDetails = (action: string, details: any, target: string) => {
    ...
    if (!d) d = {};
    if (action === 'DELETE_PRIZE') return `ID: ${target}`;
    if (action.includes('PRIZE')) return `${target}`;
    return JSON.stringify(d);
  };

  const handleUpdateStatus = async (id: string, newStatus: string) => {
    if (!window.confirm(newStatus === 'redeemed' ? t('admin_msg_confirm_payment') : t('admin_msg_refuse_request')))
      return;
    setActionLoading(true);
    try {
      await api.admin.updateRedemptionStatus(id, newStatus);
      await fetchData();
    } catch (e) {
  ...
  const handleAddSpins = async () => {
    if (selectedUser) {
      try {
        ...
        alert('Erro ao adicionar giros: ' + e.message);
      }
    }
  };
  ...
  const handleAddCoins = async () => {
    if (selectedUser) {
      try {
        ...
        alert('Erro ao adicionar coins: ' + e.message);
      }
    }
  };
  ...
  const handleBanUser = async (user: UserProfile) => {
    if (
      window.confirm(
        t('admin_msg_ban_confirm', {
          action: !user.is_banned ? t('admin_msg_ban') : t('admin_msg_unban'),
          name: user.full_name
        })
      )
    ) {
      await api.admin.banUser(user.id, !user.is_banned);
      fetchData();
    }
  };

  const handleBanIp = async (ip: string) => {
    if (window.confirm(t('admin_msg_ip_ban', { ip }))) {
  ...
  const handleDeletePrize = async (id: string) => {
    if (window.confirm(t('admin_msg_prize_delete_confirm'))) {
      setActionLoading(true);
      try {
        await api.prizes.delete(id);
        setPrizes(await api.prizes.list());
      } catch (e: any) {
        alert('Erro: ' + e.message);
      } finally {
        setActionLoading(false);
    ...
    <Container maxWidth="xl" sx={{ py: 6 }}>
      ...
      >
        <Box textAlign={isMobile ? 'center' : 'left'}>
          ...
          <Typography variant="h4" fontWeight={800} color="#D4AF37">
            {t('admin_title')}
          </Typography>
        </Box>
        <Button startIcon={<Refresh />} onClick={fetchData} variant="outlined" fullWidth={isMobile}>
          {t('btn_update')}
        </Button>
      </Box>
      ...
      <Paper sx={{ mb: 4, bgcolor: '#0F121D', borderRadius: 3, overflow: 'hidden' }}>
        ...
        >
          ...
          <Tab label="Controle de IP" icon={<Gavel sx={{ mb: 0.5 }} />} iconPosition="start" />
        </Tabs>
      </Paper>
      ...
      {loading ? (
        ...
        <>
            ...
            <Box>
              <Box display="flex" gap={2} mb={2} justifyContent="space-between" flexWrap="wrap">
                <ToggleButtonGroup
                  value={requestFilter}
                  exclusive
                  onChange={(_, v) => v && setRequestFilter(v)}
                  size="small"
                  sx={{ bgcolor: 'rgba(255,255,255,0.05)' }}
                >
                  <ToggleButton value="requested" sx={{ color: 'white' }}>
                    {t('filter_pending')}
                  </ToggleButton>
                  <ToggleButton value="redeemed" sx={{ color: 'white' }}>
                    {t('filter_paid')}
                  </ToggleButton>
                </ToggleButtonGroup>
                <Button
                  ...
                  sx={{ bgcolor: '#FFF', color: '#000', fontWeight: 800 }}
                >
                  {t('btn_print_labels')}
                </Button>
              </Box>
                      ...
                      actions={
                        <Box display="flex" gap={1} width="100%" justifyContent="flex-end">
                          <Button
                            ...
                            color="success"
                            startIcon={<WhatsApp />}
                            onClick={() => handleContactUser(req.user_details.phone, req.user_details.full_name)}
                          >
                            Contato
                          </Button>
                          {req.status === 'requested' && (
                            <>
                              <Button size="small" variant="contained" color="primary" onClick={() => handleUpdateStatus(req.id, 'redeemed')}>
                                {t('btn_pay')}
                              </Button>
                              <Button size="small" variant="outlined" color="error" onClick={() => handleUpdateStatus(req.id, 'pending')}>
                                {t('btn_refuse')}
                              </Button>
                            </>
                          )}
                      ...
                      {requests.map((req) => (
                        <TableRow key={req.id}>
                          ...
                          <TableCell sx={{ color: 'white' }}>
                            ...
                            <br />
                            <Typography variant="caption" color="text.secondary">
                              {req.user_details.cpf} | {req.user_details.phone}
                            </Typography>
                          </TableCell>
            ...
            <Box>
              ...
              <Box display="flex" gap={2} mb={3}>
                <TextField
                  ...
                  sx={{ bgcolor: 'rgba(255,255,255,0.05)' }}
                />
                <Tooltip title={sortByCoins ? 'Ordenado por Saldo (Maior)' : 'Ordenar por Saldo'}>
                  <Box component="div">
                    <ToggleButton
                      value="check"
              ...
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={4} flexDirection={isMobile ? 'column' : 'row'} gap={2}>
                <Box textAlign={isMobile ? 'center' : 'left'}>
                  ...
                  <Typography variant="body2" color="text.secondary">
                    Gerencie os itens, probabilidades e pesos.
                  </Typography>
                </Box>
                <Button
                  startIcon={<Add />}
                      ...
                      >
                        ...
                        <CardContent sx={{ pl: 3 }}>
                          <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={1}>
                            ...
                            />
                            <Chip label={p.active ? 'ATIVO' : 'INATIVO'} size="small" color={p.active ? 'success' : 'default'} variant="outlined" />
                          </Box>
                          ...
                          <Box sx={{ bgcolor: 'rgba(0,0,0,0.3)', p: 1.5, borderRadius: 2, mb: 2 }}>
                            ...
                            <LinearProgress
                              ...
                              sx={{
                                ...
                                '& .MuiLinearProgress-bar': { bgcolor: rarityColor }
                              }}
                            />
                          </Box>

                          <Box display="flex" justifyContent="flex-end" gap={1}>
                            <IconButton size="small" onClick={() => handleEditPrize(p)} sx={{ color: '#AAA', bgcolor: 'rgba(255,255,255,0.05)', '&:hover': { color: '#FFF' } }}>
                              <Edit fontSize="small" />
                            </IconButton>
                            <IconButton
                              size="small"
                              ...
                              sx={{ color: '#ff6666', bgcolor: 'rgba(255,0,0,0.1)', '&:hover': { bgcolor: 'rgba(255,0,0,0.2)' } }}
                            >
                    ...
                    <Grid key={p.id} size={{ xs: 12, md: 6 }}>
                      <Card sx={{ bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 3 }}>
                        <CardContent>
                          ...
                          <Button variant="outlined" size="small" fullWidth onClick={() => handleOpenProof(p)}>
                            {t('link_view_proof')}
                          </Button>
                        </CardContent>
                      </Card>
                  ...
                  {logs.map((log) => (
                    <MobileCard key={log.id}>
                      ...
                      <Box display="flex" justifyContent="space-between" my={1}>
                        <Typography variant="body2" color="#D4AF37" fontWeight={700}>
            ...
            <Box>
              <Paper sx={{ p: { xs: 2, md: 4 }, bgcolor: '#0F121D', border: '1px solid rgba(212,175,55,0.2)' }}>
                <Typography variant="h6" fontWeight={700} gutterBottom>
                  Configuração de Temas & Eventos
                </Typography>

                <Paper sx={{ p: 2, bgcolor: 'rgba(255, 0, 0, 0.05)', border: '1px dashed rgba(255, 0, 0, 0.3)', mb: 4, borderRadius: 2 }}>
                  <Box display="flex" alignItems="center" gap={2} mb={1}>
                    <Construction color="error" />
                    <Typography variant="subtitle1" fontWeight={700} color="error">
                      MODO MANUTENÇÃO (Sistema)
                    </Typography>
                  </Box>
                  <Typography variant="body2" color="text.secondary" mb={2}>
                    Ative esta opção para bloquear o acesso de usuários comuns. Apenas administradores conseguirão acessar a plataforma através da rota de login secreta na tela de manutenção.
                  </Typography>
                  <FormControlLabel
                    control={
                      <Switch
                        checked={localThemeConfig.maintenance_mode || false}
                        onChange={(e) => setLocalThemeConfig({ ...localThemeConfig, maintenance_mode: e.target.checked })}
                        color="error"
                      />
                    }
                    label={localThemeConfig.maintenance_mode ? '⚠️ MANUTENÇÃO ATIVA' : 'Sistema Online'}
                    sx={{ color: localThemeConfig.maintenance_mode ? '#ff6666' : 'white', '& .MuiFormControlLabel-label': { fontWeight: 700 } }}
                  />
                </Paper>
                ...
                <FormControl fullWidth sx={{ mb: 3 }}>
                  <InputLabel id="theme-select-label">{t('admin_select_theme')}</InputLabel>
                  <Select
                    labelId="theme-select-label"
                    value={localThemeConfig.name}
                    onChange={(e) => setLocalThemeConfig({ ...localThemeConfig, name: e.target.value as any })}
                    label={t('admin_select_theme')}
                  >
                    <MenuItem value="default">{t('theme_default')}</MenuItem>
                    <MenuItem value="carnival">{t('theme_carnival')}</MenuItem>
                    <MenuItem value="custom">{t('theme_custom')}</MenuItem>
                  </Select>
                </FormControl>
                <Button
                  variant="contained"
                  ...
                  disabled={actionLoading}
                  fullWidth={isMobile}
                  sx={{ mt: 2, bgcolor: '#D4AF37', color: '#000', fontWeight: 800 }}
                >
                  {t('admin_save_config')}
                </Button>
              </Paper>
          ...
          {tab === 6 && <AdminIPControl />}
        </>
      )}

      <Dialog open={prizeDialog} onClose={() => setPrizeDialog(false)} fullWidth maxWidth="sm">
        <DialogTitle>{newPrize.id ? t('admin_edit_prize') : t('new_prize')}</DialogTitle>
        <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
          ...
          <FormControl fullWidth>
            <InputLabel id="prize-type-label">{t('admin_type_label')}</InputLabel>
            <Select
              labelId="prize-type-label"
              value={newPrize.type}
              onChange={(e) => setNewPrize({ ...newPrize, type: e.target.value as any })}
          ...
          <TextField label={t('admin_image_url_label')} fullWidth value={newPrize.image_url} onChange={(e) => setNewPrize({ ...newPrize, image_url: e.target.value })} />
          <TextField label={t('admin_description_label')} fullWidth multiline rows={2} value={newPrize.description} onChange={(e) => setNewPrize({ ...newPrize, description: e.target.value })} />
        ...
        <DialogActions>
          ...
          <Button onClick={handleSavePrize} variant="contained" disabled={actionLoading}>
            {t('btn_save')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={spinsDialog} onClose={() => setSpinsDialog(false)}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          <Typography variant="body2" mb={2}>
            Usuário: <b>{selectedUser?.full_name}</b>
          </Typography>
          <TextField label={t('admin_amount_label')} type="number" fullWidth value={spinsAmount} onChange={(e) => setSpinsAmount(Number(e.target.value))} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setSpinsDialog(false)}>{t('btn_cancel')}</Button>
          <Button onClick={handleAddSpins} variant="contained">
            {t('btn_save')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={coinsDialog} onClose={() => setCoinsDialog(false)}>
        ...
        <DialogContent sx={{ pt: 2 }}>
          ...
          <TextField label={t('admin_amount_label')} type="number" fullWidth value={coinsAmount} onChange={(e) => setCoinsAmount(Number(e.target.value))} />
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setCoinsDialog(false)}>{t('btn_cancel')}</Button>
          <Button onClick={handleAddCoins} variant="contained">
            {t('btn_save')}
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={proofModalOpen} onClose={() => setProofModalOpen(false)} fullWidth maxWidth="md">
        <DialogTitle>{t('proof_analysis_title')}</DialogTitle>
          ...
          {selectedProof && (
            <Grid container spacing={2}>
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                <Typography variant="body1" gutterBottom>
                  {selectedProof.challenges?.title}
                </Typography>

                <Typography variant="subtitle2" color="text.secondary" mt={2}>
                  {t('proof_context_label')}
                </Typography>
              ...
              <Grid size={{ xs: 12, md: 6 }}>
                ...
                >
                  {selectedProof.verification_proof?.includes('IMG:') ? (
                    ...
                    <Typography color="error">{t('proof_no_image')}</Typography>
                  )}
                </Box>
        ...
        <DialogActions>
          <Button onClick={() => setProofModalOpen(false)} color="inherit">
            {t('cancel')}
          </Button>
          <Button onClick={() => selectedProof && handleMissionValidation(selectedProof.id, false)} color="error" disabled={!!processingId}>
            {t('btn_reject')}
          </Button>
          <Button
          ...
          >
            {processingId === selectedProof?.id ? t('admin_processing') : t('btn_approve')}
          </Button>
        </DialogActions>
      </Dialog>
